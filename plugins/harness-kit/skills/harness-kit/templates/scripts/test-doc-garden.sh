...
mkdir -p "$WORK/repo/scripts" "$WORK/repo/docs"
cp "$SCRIPTS_DIR/doc-garden.sh" "$WORK/repo/scripts/doc-garden.sh"
...
report=$(DOC_GARDEN_NOW=2026-07 bash "$WORK/repo/scripts/doc-garden.sh" \
...
for bad_now in 2026-00 2026-13; do
    if DOC_GARDEN_NOW="$bad_now" bash "$WORK/repo/scripts/doc-garden.sh" --repo "$WORK/repo" --format json >/dev/null 2>&1; then
...
if DOC_GARDEN_FINDINGS_FILE="$WORK/findings-is-a-directory" \
    bash "$WORK/repo/scripts/doc-garden.sh" --repo "$WORK/repo" --format json >/dev/null 2>&1; then
