...
# $WORK/scripts/harness/hooks and make $WORK a git repo with a couple of commits.
mkdir -p "$WORK/scripts/harness/hooks" "$WORK/docs/plans/active"
cp "$HOOKS_DIR/session-context.sh" "$WORK/scripts/harness/hooks/session-context.sh"
HOOK="$WORK/scripts/harness/hooks/session-context.sh"
(
    cd "$WORK" || exit 1
    git init -q .
