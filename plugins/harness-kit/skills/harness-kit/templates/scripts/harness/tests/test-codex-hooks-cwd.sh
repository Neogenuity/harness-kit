...
# `bash scripts/harness/hooks/X.sh` exits 127 from a subdir — reproduced in the PR #6
...
# lives: installed as scripts/harness/tests/test-*.sh (wiring at <root>/.codex/
# hooks.json) or shipped as templates/scripts/harness/tests/test-*.sh (wiring at
...
# Throwaway repo whose scripts/harness/hooks/ holds the real hook scripts, so a
...
git -C "$WORK" init -q
mkdir -p "$WORK/scripts/harness/hooks" "$WORK/.harness/hooks" "$WORK/deep/nested/dir"
cp "$HOOKS_DIR"/*.sh "$WORK/scripts/harness/hooks/"
# The wiring also invokes the repo-owned policy hook at .harness/hooks/;
# stage it from whichever layout this checkout provides (installed home vs
# the kit's template hooks/ dir).
for gpp in "$HOOKS_DIR/../../../.harness/hooks/guard-project-policy.sh" \
           "$HOOKS_DIR/../../hooks/guard-project-policy.sh"; do
    [ -f "$gpp" ] && { cp "$gpp" "$WORK/.harness/hooks/"; break; }
done
