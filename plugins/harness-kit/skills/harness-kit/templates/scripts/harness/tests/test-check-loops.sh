...
#   2. unit — assert_loop_ran raises an ERROR on a zero count and stays silent
#      on a positive one.
#   3. end-to-end — the real check-doctor.sh, run against a fixture with NO
...
for _f in "$LIB_SRC"/*.sh "$HOOK_SRC"/*.sh "$HARNESS_DIR"/tests/*.sh "$HARNESS_DIR"/*; do
    ...
    _hits=$(awk -v marker="$MARKER" "$scan_prog" "$_f" 2>/dev/null)
    [ -n "$_hits" ] || continue
...
# --- 3. end-to-end: a real family with nowhere to write a here-doc temp ------
