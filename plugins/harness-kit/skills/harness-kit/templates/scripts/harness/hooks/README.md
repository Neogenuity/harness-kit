...
| `guard-secrets.sh` | before a file read | Denies reads of secret-bearing files (exit code 2). Patterns come from `scripts/harness/harness.conf` (`SECRET_PATTERNS` / `SECRET_ALLOW_PATTERNS`) — the single source that `check-harness.sh` also verifies the native deny lists against. Matching is **case-insensitive** (`.ENV` reads the same bytes as `.env` on macOS/Windows) and **follows symlinks** (the target is authoritative, so a link named `notes.md` → `.env` is blocked and `.env.example` → `.env` does not launder the secret). Also denies apply_patch **writes** to secret files and token-scans shell command strings — the only live secret layer on Codex, where reads are shell commands — best-effort and bypassable (indirection, globs, encodings); directory-wide searches are **not** intercepted. Defense-in-depth, not a boundary; pair with the harness's native permission deny list. Regression-tested by `test-guard-secrets.sh` (CI-gated). |
...
- Every guard that denies or advises gets a `test-*.sh` regression script
  here, wired into `scripts/harness/check-harness` (CI-gated).
- Deny, advisory, and categorical lint-count events use the exact v2 envelope
  in `.harness/var/log.jsonl` (git-ignored); exact-v1 reviewer findings may coexist.
  Lint source/diagnostic text is fed back in-turn but never persisted. Toggle
  with `HARNESS_LOG` in `harness.conf` (env overrides win); `audit-log.sh` owns
  mixed-log rates, retries, repeat paths, review counts, and N/A states.
