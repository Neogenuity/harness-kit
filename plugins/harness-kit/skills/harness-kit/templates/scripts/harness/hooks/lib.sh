...
LOG_LIB="$(cd "$(dirname "${BASH_SOURCE[0]}")/../lib" 2>/dev/null && pwd)/log-lib.sh"
...
# (.harness/var/log.jsonl under the repo root, git-ignored) so repeated agent
# mistakes become visible data — the audit workflow summarizes it. Controlled
# by HARNESS_LOG (default on; env wins, then scripts/harness/harness.conf) and
...
hook_log() {
    ...
    [ "$enabled" = "0" ] && return 0
    logfile="${HOOK_ENV_HARNESS_LOG_FILE:-${HARNESS_LOG_FILE:-$root/.harness/var/log.jsonl}}"
...
# in .harness/var/log.jsonl even where it can't reach the model. An unrecognized
...
# .harness/var/stop-markers/ (dir overridable via HARNESS_STOP_MARKER_DIR,
