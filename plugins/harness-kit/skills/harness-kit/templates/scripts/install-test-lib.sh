...
# skip it by name; it has no case count of its own to report. It IS listed
# in this file's own _HARNESS_MECHANISM_TOPLEVEL (install-lib.sh), so it
# ships with every install and is manifest-pinned like any other mechanism
# file — #9c names it explicitly for the same reason #6 does not reach it.
#
