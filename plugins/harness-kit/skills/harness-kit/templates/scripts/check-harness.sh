...
#
#     SCOPE is deliberately exactly the set #6 executes — scripts/test-*.sh and
#     scripts/hooks/test-*.sh — not all of scripts/. The kit ships into other
#     people's repos: their scripts/deploy.sh is theirs to write, and a build gate
#     that fails their build over a scratch file it never runs is overreach. What
#     check-harness.sh RUNS, check-harness.sh may demand hygiene from; that is the
#     whole claim, and it is also the blast radius (these are the scripts that
#     `git init` throwaway repos and `rm -rf` scratch trees).
#
...
while IFS= read -r mktemp_finding; do
    ...
    for tscript in "$ROOT"/scripts/test-*.sh "$ROOT"/scripts/hooks/test-*.sh; do
        ...
        awk -v rel="${tscript#"$ROOT"/}" '
            ...
            # This is what keeps `die "mktemp failed"` and the utility-shim lists in
            # test-check-harness.sh / test-verify.sh out of the scan.
