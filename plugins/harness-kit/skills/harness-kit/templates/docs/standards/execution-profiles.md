...
config files that happen to exist, and
never replace an existing config to adopt a profile: merge the proposed fields,
preserve hooks, MCP servers, permissions, and local keys, then get explicit
approval for the diff.
...
`localhost` and `127.0.0.1` entries do not
make this a localhost-only boundary. Leave `unix_sockets` unset, and leave
