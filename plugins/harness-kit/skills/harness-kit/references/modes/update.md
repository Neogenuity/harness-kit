...
**Preflight — runtime prerequisites (before touching the install).** Source
`scripts/install-lib.sh` and run `harness_missing_prereqs`; surface anything it
prints. `jq` is the critical one — without it every guard hook fails OPEN, so
...
2. For each mechanism file: checksum matches manifest → replace with the
   ...
   `harness_update_apply` in `install-lib.sh` runs this decision
