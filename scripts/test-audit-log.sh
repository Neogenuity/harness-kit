...
mkdir -p "$WORK/repo/scripts" "$WORK/repo/.harness"
cp "$SCRIPTS_DIR/audit-log.sh" "$WORK/repo/scripts/audit-log.sh"
...
cat > "$WORK/repo/.harness/log.jsonl" <<'EOF'
...
report=$(bash "$WORK/repo/scripts/audit-log.sh" --repo "$WORK/repo" \
    --log "$WORK/repo/.harness/log.jsonl" --format json); rc=$?
...
mkdir -p "$WORK/alternate/scripts" "$WORK/alternate/.harness/eval-results" "$WORK/alternate/docs/evals"
printf '%s\n' '{"ts":"2026-07-15T11:00:00Z","hook":"code-reviewer","event":"review-finding","file":"alt","detail":"{}"}' \
    > "$WORK/alternate/.harness/log.jsonl"
printf '{}\n' > "$WORK/alternate/docs/evals/baselines.json"
cat > "$WORK/alternate/scripts/eval-harness.sh" <<'EOF'
#!/usr/bin/env bash
printf '%s\n' '{"version":1,"status":"pass","regressions":0,"violations":0,"cells":[]}'
EOF
chmod +x "$WORK/alternate/scripts/eval-harness.sh"
alternate=$(bash "$WORK/repo/scripts/audit-log.sh" --repo "$WORK/alternate" --format json); rc=$?
...
empty=$(bash "$WORK/repo/scripts/audit-log.sh" --repo "$WORK/repo" \
    --log "$WORK/repo/.harness/absent.jsonl" --format json); rc=$?
empty_table=$(bash "$WORK/repo/scripts/audit-log.sh" --repo "$WORK/repo" \
...
mkdir -p "$WORK/no-git/scripts" "$WORK/no-git/.harness"
cp "$SCRIPTS_DIR/audit-log.sh" "$WORK/no-git/scripts/audit-log.sh"
no_git=$(bash "$WORK/no-git/scripts/audit-log.sh" --repo "$WORK/no-git" --format json); rc=$?
...
mkdir -p "$WORK/shallow/.harness" "$WORK/shallow/scripts"
cp "$SCRIPTS_DIR/audit-log.sh" "$WORK/shallow/scripts/audit-log.sh"
shallow=$(bash "$WORK/shallow/scripts/audit-log.sh" --repo "$WORK/shallow" --format json); rc=$?
...
mkdir -p "$WORK/repo/.harness/eval-results" "$WORK/repo/docs/evals"
printf '{}\n' > "$WORK/repo/docs/evals/baselines.json"
cat > "$WORK/repo/scripts/eval-harness.sh" <<'EOF'
#!/usr/bin/env bash
case " $* " in *' --format json '*) ;; *) exit 2 ;; esac
printf '%s\n' '{"version":1,"status":"fail","regressions":1,"violations":0,"cells":[]}'
EOF
chmod +x "$WORK/repo/scripts/eval-harness.sh"
with_eval=$(bash "$WORK/repo/scripts/audit-log.sh" --repo "$WORK/repo" \
    --log "$WORK/repo/.harness/log.jsonl" --format json); rc=$?
...
table=$(bash "$WORK/repo/scripts/audit-log.sh" --repo "$WORK/repo" \
    --log "$WORK/repo/.harness/log.jsonl" --format table); rc=$?
