...
# own dialect — .codex uses TOML (*.toml), the rest Markdown (*.md). Same
...
if [ "$agent_providers_declared" -eq 1 ]; then
    for canonical in "$ROOT/$CANONICAL_AGENTS"/*.md; do
        ...
        for provider in $AGENT_PROVIDERS; do
            ...
            if [ "$MODE" = "--check" ]; then
                if [ ! -f "$dest" ]; then
                    echo "STALE: agent stub $dest_rel is missing — run 'bash scripts/sync-agent-skills.sh' and commit the result"
                    fail=1
                elif [ "$(cat "$dest")" != "$stub" ]; then
                    echo "STALE: agent stub $dest_rel does not match the generator output (canonical: $CANONICAL_AGENTS/$slug.md) — run 'bash scripts/sync-agent-skills.sh' and commit the result"
                    fail=1
                fi
            else
                mkdir -p "$(dirname "$dest")"
