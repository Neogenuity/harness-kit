...
if [ "$MODE" = "secrets" ]; then
    ...
    secrets_reconcile_claude() {
        ...
        if [ "$new" != "$(cat "$cfg")" ]; then
            ...
            tmp=$(mktemp "$cfg.hk-secrets.XXXXXX") || return 1
            printf '%s\n' "$new" > "$tmp" && mv "$tmp" "$cfg"
...
render_adapter() {
    ...
    # The blank line after the comment removes the collision under prettier's
    # DEFAULT config only: out of the box, prettier inserts a blank line
    # between a block-level HTML comment and a following heading, so the
    # no-blank-line bytes `sync` used to emit would immediately re-drift under
    # `--write`. It does NOT make this file formatter-invariant — the
    # hard-wrapped prose paragraph below ("Generated wiring summary ...")
    # relies on prettier's default `proseWrap: "preserve"`; a repo configured
    # for `proseWrap: "always"` or `"never"` reflows that paragraph and
    # reopens the identical deadlock (`sync` regenerates one set of bytes,
    # `--write` rewrites them, `sync --check` rejects whichever one just ran)
    # on this same file, which still has no `# tailored` escape hatch. This is
    # the same reasoning behind leaving `hooks/README.md`'s prose hand-authored
    # rather than reflowed to match one formatter config: a formatter's
    # canonical output is config-dependent, so no single hand-picked line
    # wrapping or spacing choice is universally correct.
    #
    # What actually guarantees no deadlock, for every config, is the
    # formatter-ignore block (`harness_append_formatterignore` in
    # install-lib.sh, writing `.prettierignore` — see
    # docs/standards/templates.md) excluding this file from the formatter
    # entirely. The only fix that would be formatter-invariant WITHOUT an
    # ignore entry is emitting an artifact no formatter parses at all
    # (extensionless, or a `.txt` extension instead of `.md`) — that
    # alternative was considered and rejected here (this file stays
    # human-readable Markdown) rather than missed.
