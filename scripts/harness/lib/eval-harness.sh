...
[ -d "$RESULTS_DIR" ] || die "no results dir at $RESULTS_DIR — run scripts/harness/run-evals first"
...
[ "$RESULT_COUNT" -gt 0 ] || die "no results.jsonl under $RESULTS_DIR — run scripts/harness/run-evals first"
...
#   jobs.evals.steps:
#     - run: bash scripts/harness/run-evals <task> --provider claude --model haiku --trials 3
#     - run: bash scripts/harness/lib/eval-harness.sh   # fails the job on a regression-suite drop
# ------------------------------------------------------------------------------
