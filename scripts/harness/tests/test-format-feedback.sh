...
# .harness/var/log.jsonl via hook_log (format.sh calls it before hook_feedback).
...
R="$WORK/repo"
mkdir -p "$R/scripts/harness/hooks"
cp "$HOOKS_DIR/format.sh" "$HOOKS_DIR/lib.sh" "$R/scripts/harness/hooks/"
cp "$HOOKS_DIR/../lib/log-lib.sh" "$R/scripts/harness/lib/log-lib.sh"
chmod +x "$R/scripts/harness/hooks/format.sh"
printf 'LINT_RULES="*.zzz=./zzz-linter"\nFORMAT_RULES="*.zzz=./zzz-formatter"\n' > "$R/scripts/harness/harness.conf"
...
fmt_err=$(printf '{"session_id":1,"tool_input":{"file_path":"thing.zzz"}}' \
    | "$R/scripts/harness/hooks/format.sh" 2>&1 >/dev/null)
fmt_rc=$?
