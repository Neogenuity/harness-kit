...
mkdir -p "$WORK/scripts/harness/hooks" "$WORK/src"
cp "$HOOKS_DIR/lib.sh" "$WORK/scripts/harness/hooks/lib.sh"
cp "$HOOKS_DIR/guard-config.sh" "$WORK/scripts/harness/hooks/guard-config.sh"
HOOK="$WORK/scripts/harness/hooks/guard-config.sh"
...
run 2 "hook script edit denied"          "$(payload "$WORK/scripts/harness/hooks/lib.sh")"
run 2 "check-harness.sh edit denied"     "$(payload "$WORK/scripts/harness/check-harness")"
run 2 "install-lib.sh edit denied"       "$(payload "$WORK/scripts/harness/lib/install-lib.sh")"
run 2 "dev-instance.sh edit denied"      "$(payload "$WORK/scripts/harness/lib/dev-instance.sh")"
run 2 "project dev.sh edit denied"       "$(payload "$WORK/scripts/dev.sh")"
run 2 "eval runner edit denied"          "$(payload "$WORK/scripts/harness/run-evals")"
run 2 "eval-lib edit denied"             "$(payload "$WORK/scripts/harness/lib/eval-lib.sh")"
run 2 "manifest edit denied"             "$(payload "$WORK/scripts/harness/.harness-manifest")"
run 2 "regression test edit denied"      "$(payload "$WORK/scripts/harness/tests/test-check-harness.sh")"
...
run 2 "relative path denied"             "$(payload "scripts/harness/hooks/guard-secrets.sh")"
run 2 "Cursor layout denied"             "$(cursor_payload "$WORK/scripts/harness/sync")"

# --- deny: 2026-07-12 probe-confirmed gaps (harness.conf, local settings, MCP configs) ---
run 2 "harness.conf edit denied"          "$(payload "$WORK/scripts/harness/harness.conf")"
...
run 2 "Codex patch: hook edit denied"            "$(codex_patch '*** Update File: scripts/harness/hooks/lib.sh
@@
+x')"
run 2 "Codex patch: added protected file denied" "$(codex_patch '*** Add File: scripts/harness/hooks/evil.sh
+#!/usr/bin/env bash')"
run 2 "Codex patch: rename onto mechanism denied" "$(codex_patch '*** Update File: src/x.sh
*** Move to: scripts/harness/check-harness')"
run 2 "Codex patch: multi-file, manifest second" "$(codex_patch '*** Update File: src/app.php
@@
+x
*** Update File: scripts/harness/.harness-manifest
@@
+y')"
run 2 "Codex patch: dot-slash prefixed path denied" "$(codex_patch '*** Update File: ./scripts/harness/hooks/lib.sh
@@
+x')"
...
run 2 "Codex bare patch: hook edit denied"       "$(codex_patch_bare '*** Update File: scripts/harness/hooks/lib.sh
@@
+x')"
run 2 "Codex bare patch: manifest edit denied"   "$(codex_patch_bare '*** Update File: scripts/harness/.harness-manifest
@@
+x')"
...
run 2 "Codex patch: harness.conf edit denied"       "$(codex_patch '*** Update File: scripts/harness/harness.conf
@@
+x')"
run 2 "Codex bare patch: harness.conf edit denied"  "$(codex_patch_bare '*** Update File: scripts/harness/harness.conf
@@
+x')"
run 2 "Codex patch: dev-instance edit denied"       "$(codex_patch '*** Update File: scripts/harness/lib/dev-instance.sh
@@
+x')"
...
run 0 "Codex shell: sed on mechanism not scanned (documented limit)" "$(jq -cn '{tool_input: {command: "sed -i s/a/b/ scripts/harness/hooks/lib.sh"}}')"
...
run 0 "Codex shell: patch text in a heredoc not treated as a mechanism edit" "$(jq -cn --arg c "$(printf 'cat > demo.patch <<PATCH\n*** Begin Patch\n*** Update File: scripts/harness/hooks/lib.sh\n@@\n+evil\n*** End Patch\nPATCH')" '{turn_id: "t1", tool_name: "shell", tool_use_id: "c1", tool_input: {command: $c}}')"
run 0 "Codex patch: escape hatch allows mechanism edit" "$(codex_patch '*** Update File: scripts/harness/hooks/lib.sh
@@
+x')" "HARNESS_ALLOW_MECHANISM_EDITS=1"
run 0 "Codex patch: escape hatch allows dev-instance edit" "$(codex_patch '*** Update File: scripts/harness/lib/dev-instance.sh
@@
+x')" "HARNESS_ALLOW_MECHANISM_EDITS=1"
...
# globs (`scripts/harness/./harness.conf` resolves to
# `scripts/harness/harness.conf`). Purely lexical; NOT case-folding — a case
# variant on a case-insensitive FS stays a documented guardrail limitation
# caught by the CI manifest layer.
run 2 "dot-slash harness.conf denied"          "$(payload "$WORK/scripts/harness/./harness.conf")"
run 2 "dot-dot harness.conf denied"            "$(payload "$WORK/scripts/../scripts/harness/harness.conf")"
run 2 "dot-slash settings.local denied"        "$(payload "$WORK/.claude/./settings.local.json")"
run 2 "dot-slash hook script denied"           "$(payload "$WORK/scripts/harness/hooks/./lib.sh")"
...
run 2 "Codex patch dot-slash mid-path denied"  "$(codex_patch '*** Update File: scripts/harness/./harness.conf
@@
+x')"
run 2 "Codex patch dot-dot mid-path denied"    "$(codex_patch '*** Update File: scripts/../scripts/harness/.harness-manifest
@@
+x')"
...
out=$(_hintout "$(payload "$WORK/scripts/harness/harness.conf")")
...
out=$(_hintout "$(payload "$WORK/scripts/harness/harness.conf")" 'GUARD_DENY_HINT=EDIT-THE-TEMPLATE')
...
out=$(_hintout "$(payload "$WORK/scripts/harness/hooks/lib.sh")")
...
rm -f "$WORK/scripts/harness/harness.conf"
...
run 0 "escape hatch allows mechanism edit" "$(payload "$WORK/scripts/harness/hooks/lib.sh")" "HARNESS_ALLOW_MECHANISM_EDITS=1"
run 0 "escape hatch allows harness.conf tailoring" "$(payload "$WORK/scripts/harness/harness.conf")" "HARNESS_ALLOW_MECHANISM_EDITS=1"
run 0 "escape hatch allows direct dev-instance edit" "$(payload "$WORK/scripts/harness/lib/dev-instance.sh")" "HARNESS_ALLOW_MECHANISM_EDITS=1"
