...
# `git checkout scripts/harness/hooks/lib.sh` and every mention of a protected
# path), so shell edits (`sed -i` via Bash) are not intercepted;
# scripts/harness/check-harness's manifest verification is the enforcing layer in
...
# scripts/harness/.harness-manifest — the same ceremony covers post-init harness.conf
...
# scripts/harness/* covers the entire kit tree — commands, lib/, hooks/,
# tests/, both manifests, and harness.conf (guard-secrets.sh's single pattern
# source; post-init tailoring rides the HARNESS_ALLOW_MECHANISM_EDITS=1
# escape hatch above, same ceremony as any other entry). The .harness/
# entries are the repo's own enforcement-layer policy — the verify gate list
# and the project stop hook — which an agent could otherwise edit to fake a
# green verify.
PROTECTED_PATHS="
scripts/harness/*
scripts/dev.sh
...
.github/workflows/*
"
# 2026-07-12 probe found these writable live, each a cheap way to disarm
# another layer before the CI manifest check catches it:
...
#   check-harness's MCP inventory audit (#8c); opencode.json is already
#   covered above.
...
# `scripts/./harness.conf` or `scripts/../scripts/harness/harness.conf` can't slip past
...
check_file() {
    ...
    for pat in $PROTECTED_PATHS; do
        ...
        if [ "$hit" = "1" ]; then
            hook_deny "Blocked by scripts/harness/hooks/guard-config.sh: '$rel' is harness mechanism or a protected config. If a check is failing, fix the code it complains about — do not edit the check. Intentional harness maintenance: re-run with HARNESS_ALLOW_MECHANISM_EDITS=1, then re-pin scripts/harness/.harness-manifest (see check-harness.sh).${DENY_HINT:+ $DENY_HINT}"
        fi
    done
    return 0
}
