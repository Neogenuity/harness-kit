...
sed -i.bak 's/^SECRET_PATTERNS="\(.*\)"$/SECRET_PATTERNS="\1 *.key"/' "$TPL/scripts/harness/harness.conf"
rm -f "$TPL/scripts/harness/harness.conf.bak"
...
printf '\n# *.key covered as a secret pattern (audit reference)\n# case: expect_deny "app/server.key"\n' \
    >> "$TPL/scripts/harness/tests/test-guard-secrets.sh"
