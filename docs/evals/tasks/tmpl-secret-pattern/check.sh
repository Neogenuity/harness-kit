...
grep -qE '^SECRET_PATTERNS=.*\*\.key' "$TPL/scripts/harness.conf" 2>/dev/null && tpl_has=1
grep -qE '^SECRET_PATTERNS=.*\*\.key' scripts/harness.conf 2>/dev/null && root_has=1
...
if [ "$root_has" -eq 1 ]; then
    ...
    if ! env HARNESS_NESTED_FIXTURE=1 bash scripts/check-harness.sh >/dev/null 2>&1; then
        echo "UNREPINNED: root scripts/harness.conf edited but check-harness.sh fails (manifest not re-pinned)"
        exit 1
    fi
