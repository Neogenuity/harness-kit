...
self-protection gaps with a confirmed reproduction: `scripts/harness/harness.conf`
...
1. **Close the guard-coverage gaps** — extend the default `PROTECTED_PATHS`
   in
   [guard-config.sh](../../../plugins/harness-kit/skills/harness-kit/templates/scripts/harness/hooks/guard-config.sh)
   (template first, then roll into the installed copy) with
   `scripts/harness/harness.conf`, `.claude/settings.local.json`, `.mcp.json`,
...
2. **Repair the Cursor feedback arm** — verify the current response schema
   ...
   [lib.sh](../../../plugins/harness-kit/skills/harness-kit/templates/scripts/harness/hooks/lib.sh)
...
All five `scripts/harness/verify` gates green on the release commit (shellcheck,
...
`printf '{"tool_input":{"file_path":"scripts/harness/harness.conf"}}' | bash scripts/harness/hooks/guard-config.sh`
exits 2, likewise for `.claude/settings.local.json`, `.mcp.json`, and the
crafted `scripts/./harness.conf` / `scripts/../scripts/harness/harness.conf`
...
- 2026-07-12 — **All five scope items implemented and integrated (green).**
  ...
  intercepted), re-pinned `scripts/harness/.harness-manifest` at 0.10.1 (tailored
  markers preserved, new test auto-included). `bash scripts/harness/verify` green
...
- 2026-07-12 — **Marker-file loop guard (item 3b) stays a FALLBACK, not the
  ...
  `.harness/var/stop-markers/`, keyed on the Stop payload's session/conversation
...
ceremony ([docs/skills/release/SKILL.md](../../../.agents/skills/release/SKILL.md)):
