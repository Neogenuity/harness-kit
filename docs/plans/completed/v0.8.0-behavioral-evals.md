...
2. **Runner**: `scripts/harness/run-evals <task> [--trials N]` — runs one golden task
   ...
   trial** under `.harness/var/eval-results/<task>/<run>/` (agent output, tool
...
3. **Harness regression**: `scripts/harness/lib/eval-harness.sh` — runs the task set,
...
- **Per-trial transcripts** land under `.harness/var/eval-results/<task>/<run>/trial-N/`
  (git-ignored).
...
- 2026-07-11 — **Pre-release multi-reviewer pass (2 Claude subagents +
  ...
  by `test-eval.sh`'s reference-pass check). `bash scripts/harness/verify` green.
- 2026-07-11 — **Shipped as v0.8.0.** Live baselines recorded on two providers,
  ...
  makes `eval-harness.sh` exit 1 naming the regression. `bash scripts/harness/verify`
...
- 2026-07-11 — **Machinery built and validated offline; live baselines
  ...
  `templates/scripts/harness/lib/eval-lib.sh` (model-free core: TASK.md parsing, per-trial
  ...
  under `.harness/var/eval-results/`, `claude|codex|mock` providers), `eval-harness.sh`
  ...
  nested-skip updated; SKILL init/audit + AGENTS.md integrated. `bash scripts/harness/verify`
