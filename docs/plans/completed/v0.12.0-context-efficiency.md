...
1. **Eval usage instrumentation** — extend `eval_result_json` (and eval.sh's
   ...
   (`templates/scripts/harness/run-evals`, `eval-lib.sh`, `test-eval.sh`), rolled into
...
3. **guard-config deny-hint TAILOR slot** — the deny message gains a
   ...
   `test-guard-config.sh` covers the hint; a root `scripts/harness/harness.conf` edit
...
`bash scripts/harness/verify` green after every item (template tests + manifest
re-pin discipline per [../conventions/templates.md](../../standards/templates.md));
...
release cut per
[../skills/release/SKILL.md](../../../.agents/skills/release/SKILL.md) with version bump
and root roll-in.

## Progress

- 2026-07-13 — **Shipped as v0.12.0 (items 1–6).** Usage instrumentation,
  AGENTS.md skill-link line, `GUARD_DENY_HINT`, `/opencode.json` protected-path
  anchor, `BANNER_RECENT_COMMITS` trim, and the stop-hook clean-tree skip all
  landed with tests + root roll-in + manifest re-pin; the Codex-P1 cron
  `--model` argv fix rode along. Item 2's flip-to-pass is confirmed by the
  recorded matrix — `hn-add-skill` Claude 3/3 (audit was 0/6), and
  `tmpl-secret-pattern` now saturates 3/3 under the shipped harness (audit was
  0/3→2/3). **Item 7 (plugin skill-split) deferred** to the queued
  [v0.13.0-skill-split.md](v0.13.0-skill-split.md): it needs its own fresh paired
  monolith-vs-split parity round, and per this plan's own cut line A–D ship as
  v0.12.0 without it. The config-aware audit harness
  (`.harness/context-efficiency-eval/scripts/`) is still on disk for that run.
