...
- `scripts/harness/harness.conf` — the single source for `SECRET_PATTERNS` (plus
...
1. **Extract the deterministic mechanics** into a testable unit. `init` and
   ...
   (e.g. `templates/scripts/harness/lib/install-lib.sh`) the prose flow calls and tests
   ...
   **Manifest coverage** (the new mechanism files must not escape integrity
   ...
   file list — `scripts/harness/hooks`, `sync-agent-skills.sh`, `check-harness.sh`,
...
2. **Fixture harness** built on the plans-machinery fixture recipe: a helper
   ...
   the scratch dir.*
3. **The five deterministic cases** (`scripts/harness/tests/test-install.sh`, wired into
...
6. **Close the `harness.conf` integrity blind spot** (verified finding 1;
   ...
   `scripts/harness/harness.conf` to the *same* manifest-producer enumeration (SKILL.md
   ...
   `scripts/harness/harness.conf` to `guard-config.sh`'s `PROTECTED_PATHS` for a live
   ...
   generated manifest lists `scripts/harness/harness.conf`; mutating it without
...
7. **Make a missing manifest an ERROR post-adoption** (verified finding 2;
   ...
   silently skips its entire checksum block when `scripts/harness/.harness-manifest` is
   ...
   when the harness is clearly adopted (e.g. `scripts/harness/hooks/` present) but
...
`bash scripts/harness/verify` green including `test-install.sh`; the five
...
`scripts/harness/harness.conf` (mutating any without re-pinning fails
...
(now including `scripts/harness/harness.conf`).
...
- 2026-07-11 — **Multi-model review round (Claude subagents + Codex/gpt-5.6-terra),
  ...
  `scripts/harness/hooks/`). (2) *update correctness* — `harness_update_apply` now treats
...
- 2026-07-11 — **Shipped as v0.7.0.** All seven scope items landed. Extracted
  the deterministic mechanics into `templates/scripts/harness/lib/install-lib.sh`
  ...
  call it. New `templates/scripts/harness/tests/test-install.sh` drives the five deterministic
  ...
  still running every guard behavioral test inside the fixture. Pinned `scripts/harness/harness.conf` (` # tailored`) in the producer and this
  repo's manifest, added `scripts/harness/lib/install-lib.sh` to `guard-config.sh`'s
  ...
  makes the mechanism-upgrade case fail loudly. `bash scripts/harness/verify` green.
- 2026-07-11 — Folded in two *verified* self-protection findings from a second
  ...
  acting (scope 6–7): (1) `scripts/harness/harness.conf` is pinned by neither the
...
is a [behavioral-evals](v0.8.0-behavioral-evals.md) golden task.
