...
`.claude/settings.json` leaves `bash scripts/harness/check-harness` exiting 0 —
...
6. **Per-provider acceptance floors for recorded eval cells** — both Codex
   ...
   *Acceptance: a collision test proves a bare and a plugin-activated cell
   ...
   [../../../.harness/evals/baselines.json](../../../.harness/evals/baselines.json); the new cells are
   recorded at the standard trial count; the decision lands in this plan's
   Decisions log and in [../../../.harness/evals/README.md](../../../.harness/evals/README.md) if the
   policy changes.*

## Out of scope

- Byte-pinning provider configs in `scripts/harness/.harness-manifest` — they are
...
[../../standards/templates.md](../../standards/templates.md).
...
- `bash scripts/harness/verify` **green end to end** (shellcheck, manifests,
