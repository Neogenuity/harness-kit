...
Harness-kit itself is not an application, so the root installation intentionally
has no `scripts/dev.sh`. This canonical dogfood copy documents the contract
exercised by the root-only `verify-live-runtime` fixture; its setup installs the
fixture adapter into an isolated eval workspace. Runtime audit is otherwise N/A
for this repository.

## Runtime map

| Concern | Harness-kit fixture |
| --- | --- |
| Boot command | `scripts/dev.sh up` launches `python3 live_app/app.py` |
| Readiness check | `GET /health` returns this instance with `status: "ready"` |
